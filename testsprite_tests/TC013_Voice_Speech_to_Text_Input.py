...
async def run_test():
    ...
    try:
        ...
        # Navigate to or open the chat interface where speech-to-text input can be activated.
        frame = context.pages[-1]
        ...
        await page.wait_for_timeout(3000); await elem.fill('Test Speech Bot')
        

        frame = context.pages[-1]
        elem = frame.locator('xpath=html/body/div/div/div/main/div/div[2]/form/div/div/div[3]/input').nth(0)
        await page.wait_for_timeout(3000); await elem.fill('A bot to test speech-to-text functionality.')
        ...
        await page.wait_for_timeout(3000); await elem.fill('https://your-automation.com/webhook/test-speech-bot')
        ...
        # Click the 'Create Bot' button to submit the bot creation form and create the bot.
        frame = context.pages[-1]
        elem = frame.locator('xpath=html/body/div/div/div[3]/div/div/div[2]/a/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # Add a first node to the conversation flow to enable bot interaction, then save the bot.
        frame = context.pages[-1]
        elem = frame.locator('xpath=html/body/div/div/div[3]/div/div[3]/div[2]/div/div/div/div[2]/div/select').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # Click the 'Save' button to save the bot configuration and then proceed to test the speech-to-text input in the chat interface.
        frame = context.pages[-1]
        elem = frame.locator('xpath=html/body/div/div/div[3]/div/div[3]/div/div/div[2]/button[3]').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # Locate and activate the speech-to-text input feature in the chat interface to test spoken input transcription.
        frame = context.pages[-1]
        elem = frame.locator('xpath=html/body/div/div/div[3]/div/div[3]/div/div/div/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        assert False, 'Test plan execution failed: speech-to-text transcription verification not completed.'
