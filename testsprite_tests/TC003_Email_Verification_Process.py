...
async def run_test():
    ...
    try:
        ...
        # Fill in Full Name, Email, Password, Confirm Password fields and submit the form to create a new account.
        frame = context.pages[-1]
        ...
        # Click on 'Sign in' link to navigate to the login page for existing user authentication.
        frame = context.pages[-1]
        elem = frame.locator('xpath=html/body/div/div/div/div/div/div[2]/div[2]/p/a').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # Input email and password for existing user and click Sign In button.
        frame = context.pages[-1]
        ...
        # Click on 'Switch Account' button to explore account management options that may include email verification status or resend link.
        frame = context.pages[-1]
        elem = frame.locator('xpath=html/body/div/div/header/div/div[2]/div/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # Click on 'Account Settings' button to check for email verification status or resend verification email option.
        frame = context.pages[-1]
        elem = frame.locator('xpath=html/body/div/div/div/div/div[2]/div[3]/div/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # Scroll down to check for any email verification status message or resend verification email button.
        await page.mouse.wheel(0, window.innerHeight)
        

        # Assert that the email verification status is shown as verified in the Account Settings section.
        frame = context.pages[-1]
        verification_status_locator = frame.locator("text=Email Verified")
        assert await verification_status_locator.is_visible(), "Email verification status 'Email Verified' is not visible."
